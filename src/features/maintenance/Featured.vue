...
<template>
    ...
    <section class="w-full py-10">
        <div class="max-w-[1120px] mx-auto">
            ...
            <div class="mt-5 grid gap-8 sm:grid-cols-2 lg:grid-cols-3 select-none">
                <BaseCard v-for="(card, index) in items" :key="index"
                    ...
                    <h3 class="mt-2 text-2xl font-GarnettSemibold font-semibold text-amBluePremium leading-[34px]">
                        {{ card.title }}
                    </h3>

                    <img :src="card.img" alt="" class="mt-[13px] h-36 w-full rounded-md object-cover" />

                    <p class="mt-5 text-sm leading-5 text-amTextGray">
                        {{ card.text }}
                    </p>

                    <BaseButton class="mt-[19px] mb-5" size="sm">
                        PDF
                    </BaseButton>
                </BaseCard>
            </div>
